import type { AccountMetadataRequest, AccountMetadataRow } from '../types/imports'

const defaultApiBaseUrl = window.location.protocol === 'file:' ? 'http://127.0.0.1:8000' : ''
const API_BASE_URL = (import.meta.env.VITE_API_BASE_URL ?? defaultApiBaseUrl).replace(/\/$/, '')

async function postImport<TRequest, TResponse>(path: string, payload: TRequest): Promise<TResponse> {
